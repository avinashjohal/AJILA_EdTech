...
function InternshipDetail() {
  useEffect(() => {
    document.title = "AJILA | Internship Detail Page"
  }, [])

  const [chapterData, setChapterData] = useState([]);
  const [internshipData, setinternshipData] = useState([]);
  const [relatedinternshipData, setrelatedinternshipData] = useState([]);
  const [teacherData, setteacherData] = useState([]);
  const [userLoginStatus, setuserLoginStatus] = useState(null);
  const [enrollStatus, setenrollStatus] = useState(null);


  let { internship_id } = useParams();
  const studentId = localStorage.getItem('studentId'); 

  useEffect(() => {
    try {
      axios.get(`${baseUrl}/internship/${internship_id}/`)
        .then((response) => {
          setinternshipData(response.data);
          setChapterData(response.data.internship_chapters);
          setteacherData(response.data.teacher);
          setrelatedinternshipData(JSON.parse(response.data.related_videos));
        })
    } catch (error) {
    ...
    // fetch enroll status 
    try {
      axios.get(`${baseUrl}/fetch-internship-enroll-status/${studentId}/${internship_id}`)
        .then((response) => {
  ...
  }, []);// Add internship_id as a dependency
  ...
  const enrollInternship = () => {
    ...
    formData.append("internship", internship_id);
    formData.append("student", studentId);

    axios.post(`${baseUrl}/student-enroll-internship/`, formData, {
      headers: {
  ...
  return (
    <div className="container mt-3">
      <div className="row">
        <div className="col-4">
          <img src={internshipData.featured_img} className="img-thumbnail" alt={internshipData.title} />
        </div>
        <div className="col-8">
          ...
          <p className="fw-bold">Techs: {internshipData.techs}</p>
          <p className="fw-bold"> Duration:{internshipData.duration}</p>
          ...
          { enrollStatus !== 'success' && userLoginStatus === 'success' && (
            <p><button type="submit" onClick={(event) => enrollInternship(event)} className="btn btn-md btn-success">Enroll Now</button></p>
          )}
      ...
      <div className="row ">
        {relatedinternshipData.map((rtraining, index) =>
